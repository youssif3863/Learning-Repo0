...
CENTRAL = {
 ...
 "FI-02": dict(decision="exclude", reason="Capsule duplicate check: already taught by the earlier Internal Medicine companion (branch claude/credits-expiring-soon-7nbhf4, entry C5-15: 'Poultry -> Salmonella', from GIT Internal Med p. 55)."),
 "TU-12": dict(new_contribution="The sequence continues past intestinal metaplasia to dysplasia and then cancer; GIT Others classes intestinal metaplasia as a precancerous (dysplastic) lesion of the stomach.",
               reason="Capsule duplicate check: the earlier Internal Medicine companion (entry C3-14) already supplies 'atrophic gastritis -> intestinal metaplasia', and the book p.72 has 'on top of intestinal metaplasia'. Only the dysplasia step and the precancerous classification are new."),
}
...
for ch in C["chapters"]:
    for it in ch["items"]:
        for lid in it["ledger"]:
            entry[lid] = (it["id"], it["subject"])
for r in rows:
    r.setdefault("central_review", "confirmed")
    e = entry.get(r["id"])
    r["handout_entry"] = e[0] if e else ""
    r["subject"] = e[1] if e else {"PH": "Pharmacology", "TU": "Pathology", "FI": "Microbiology", "PA": "Parasitology"}[r["id"][:2]]
...
cols = ["id", "decision", "handout_entry", "subject", "others_page", "others_quote", "others_quote_check",
...
md = ["# Other Subjects Capsule: comparison ledger (GIT Others vs YMnotes Digestive System, Version 3, both files)", "",
      ...
      f"**Totals:** {len(rows)} candidates · {n('include')} include · {n('discrepancy')} discrepancy · {n('exclude')} exclude. The standard is in `INCLUSION-STANDARD.md`; the capsule applies it to the whole book (both files) and also excludes points already taught by the earlier Internal Medicine companion. `central_review` marks rows changed at the final central review.", "",
