...
pdf = P / "YMnotes-Digestive-After-Mid-Module-Extras.pdf"
...
rows = []
for ch in C["part_a"]["chapters"] + C["part_b"]["chapters"]:
    num = f"{ch['n']:02d}"
...
idx = d[[i for i in range(d.page_count) if "Points to check" in d[i].get_text() and "Entry" in d[i].get_text()][0]]
